...
# Import your custom high-performance module
from pygma import (
    GMAManager, 
    GMABannerAd, 
    GMAInterstitialAd, 
...
class MainUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # UI Elements
        self.status_label = Label(text="Initializing Ads...", font_size='20sp', size_hint_y=0.2)
        self.add_widget(self.status_label)
        
        self.btn_interstitial = Button(text="Show Interstitial (Loading...)", disabled=True)
        self.btn_interstitial.bind(on_press=self.show_interstitial)
        self.add_widget(self.btn_interstitial)
        
        self.btn_rewarded = Button(text="Watch Video for Coins (Loading...)", disabled=True)
...
class MyAdGameApp(App):
    ...
    def on_start(self):
        ...
        # 3. Pre-load the fullscreen ads immediately so they are ready to show
        self.interstitial_ad.load()
        self.rewarded_ad.load()
        
        # Enable buttons (in a real app, you'd check `.is_loaded()` before enabling)
